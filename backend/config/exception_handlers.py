    ...
    if response is not None:
        ...
        if isinstance(exc, AuthenticationFailed):
            ...
            if isinstance(response.data, dict) and "error" in response.data:
                ...
                custom_response_data["message"] = str(
                    response.data.get("detail", "Invalid credentials")
                )
            ...
            if isinstance(response.data, dict):
                ...
                if "detail" not in response.data:
                    ...
                    custom_response_data["message"] = str(
                        response.data.get("detail", "Invalid request data")
                    )

        elif response.status_code == status.HTTP_401_UNAUTHORIZED:
            custom_response_data["error"] = "UNAUTHORIZED"
            custom_response_data["message"] = str(
                response.data.get("detail", "Authentication required")
            )
            ...
            custom_response_data["message"] = str(
                response.data.get("detail", "Request entity too large")
            )
