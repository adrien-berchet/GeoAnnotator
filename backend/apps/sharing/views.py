...
from .services import ShareService, PermissionService, EmailInvitationService
...
class ShareViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, point_pk=None):
        ...
        serializer = CreateShareSerializer(
            data=request.data,
            context={'request': request, 'gps_point': point}
        )
        ...
        if permission_hierarchy.get(requested_permission, 0) >= permission_hierarchy.get(user_permission, 0):
            return Response(
    ...
    def retrieve(self, request, pk=None, point_pk=None):
        ...
        if not (PermissionService.is_owner(share.gps_point, request.user) or
                share.recipient_email == request.user.email):
            return Response(
    ...
    def partial_update(self, request, pk=None, point_pk=None):
        ...
        serializer = UpdateShareSerializer(share, data=request.data, partial=True, context={'request': request})
        serializer.is_valid(raise_exception=True)
