...
class PointType(models.Model):
    ...
    creation_language = models.CharField(
        max_length=10, default="en", help_text="ISO 639-1 language code at creation"
    )

    icon = models.TextField(default="📍", help_text="Icon URL, emoji, data URI, or asset reference")

    order = models.IntegerField(default=0, help_text="Display order (lower values first)")
    ...
    def clean(self):
        ...
        if not self.names or len(self.names) == 0:
            raise ValidationError({"names": "At least one translation must be provided."})
        ...
        if self.type_choice == "base" and self.owner is not None:
            raise ValidationError({"owner": "Base types cannot have an owner."})

        # Validate custom types have owner
        if self.type_choice == "custom" and self.owner is None:
            raise ValidationError({"owner": "Custom types must have an owner."})

        # Validate max types per user
        if (
            self.owner and self._state.adding
        ):  # Only check on creation (use _state.adding since pk has default value)
            active_types_count = PointType.objects.filter(owner=self.owner, status="active").count()
    ...
    def get_default_type(cls):
        ...
        default_type = cls.objects.filter(
            owner__isnull=True, type_choice="base", creation_language="en"
        ).first()
...
class Tag(models.Model):
    ...
    id = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False, help_text="Unique tag identifier"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                functions.Lower("name"), "owner", name="unique_tag_name_per_user"
            )
        ]
        indexes = [
...
class GPSPoint(models.Model):
    ...
    id = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False, help_text="Unique point identifier"
    )

    title = models.CharField(max_length=255, db_index=True, help_text="Point title (required)")

    description = models.TextField(
        blank=True, null=True, help_text="Rich text HTML description with emoticons"
    )
    ...
    tags = models.ManyToManyField(
        Tag, blank=True, related_name="points", help_text="Categorization tags"
    )
    ...
    editing_lock_acquired_at = models.DateTimeField(
        null=True, blank=True, help_text="Lock acquisition timestamp"
    )
