...
class TestPointTypeContract:
    ...
    def test_create_type_success(self, authenticated_client_alice):
        ...
        data = {"names": {"en": "Restaurant"}, "icon": "/icons/restaurant.svg", "order": 1}
    ...
    def test_create_type_duplicate_name_same_user(self, authenticated_client_alice):
        ...
        assert "A point type with these names already exists." == str(
            response.data["details"]["names"][0]
        )
    ...
    def test_list_types_only_own_types(
        self, authenticated_client_alice, authenticated_client_bob, alice, bob
    ):
    ...
    def test_list_types_excludes_deleted(self, authenticated_client_alice, alice):
        ...
        PointType.objects.create(names={"en": "Active"}, owner=alice, order=1)
        PointType.objects.create(names={"en": "Deleted"}, owner=alice, order=2, status="deleted")
    ...
    def test_get_type_detail_success(self, authenticated_client_alice, alice):
        ...
        point_type = PointType.objects.create(
            names={"en": "Museum"}, icon="/icons/museum.svg", owner=alice, order=1
        )
    ...
    def test_update_type_success(self, authenticated_client_alice, alice):
        ...
        point_type = PointType.objects.create(
            names={"en": "Restaurant"}, icon="/icons/restaurant.svg", owner=alice, order=1
        )

        url = reverse("point-types:detail", args=[point_type.id])
        data = {"names": {"en": "Café"}, "icon": "/icons/cafe.svg"}
    ...
    def test_delete_type_switches_points_to_default(self, authenticated_client_alice, alice):
        ...
        point = GPSPoint.objects.create(
            title="Test Point", location=Point(2.3522, 48.8566), owner=alice, type=custom_type
        )
