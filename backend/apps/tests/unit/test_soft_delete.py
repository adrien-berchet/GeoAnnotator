...
class TestSoftDelete:
    ...
    def test_active_user_has_null_deleted_at(self, alice):
        """Test that active users have deleted_at=None."""
        assert alice.deleted_at is None

    def test_setting_deleted_at_marks_user_deleted(self, alice):
    ...
    def test_deleted_users_excluded_from_active_manager(self, alice):
    ...
    def test_deleted_users_accessible_via_objects(self, alice):
    ...
    def test_deleted_at_timestamp_preserved(self, alice):
        """Test that deleted_at timestamp is preserved accurately."""
        deletion_time = timezone.now()
        alice.deleted_at = deletion_time
        alice.save()

        alice.refresh_from_db()

        # Should be within 1 second (accounting for DB precision)
        assert abs((alice.deleted_at - deletion_time).total_seconds()) < 1

    def test_multiple_users_different_deletion_times(self, alice, bob):
        """Test that multiple users can have different deletion times."""
        time1 = timezone.now()
        time2 = time1 + timedelta(hours=1)

        alice.deleted_at = time1
        alice.save()

        bob.deleted_at = time2
        bob.save()

        alice.refresh_from_db()
        bob.refresh_from_db()

        assert alice.deleted_at != bob.deleted_at

    def test_active_manager_only_returns_active_users(self, alice, bob):
        ...
        assert User.active.first().id == bob.id

    def test_objects_returns_all_users(self, alice, bob):
    ...
    def test_deleted_user_can_be_retrieved_by_id_via_objects(self, alice):
    ...
    def test_filter_deleted_users_explicitly(self, alice, bob):
        ...
        assert deleted_users.first().id == alice.id

    def test_filter_active_users_explicitly(self, alice, bob):
        ...
        assert active_users.first().id == bob.id

    def test_deleted_at_future_date_allowed(self, alice):
        """Test that deleted_at can be set to future date (for scheduled deletion)."""
        future_time = timezone.now() + timedelta(days=30)
        alice.deleted_at = future_time
        alice.save()

        alice.refresh_from_db()
        assert alice.deleted_at > timezone.now()

    def test_clearing_deleted_at_restores_user(self, alice):
        ...
        alice.deleted_at = None
        alice.save()

        assert User.active.filter(id=user_id).exists()

    def test_deleted_users_excluded_from_active_count(self, alice, bob):
    ...
    def test_deleted_users_excluded_from_active_exists(self, alice):
    ...
    def test_get_deleted_user_via_active_raises_does_not_exist(self, alice):
    ...
    def test_deleted_at_timezone_aware(self, alice):
        ...
        assert alice.deleted_at.tzinfo is not None
