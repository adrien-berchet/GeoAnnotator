...
describe('AnnotationList - Integration Tests', () => {
  ...
  describe('Full User Flow', () => {
    it('should render multiple annotations with different markdown content independently', async () => {
      ...
      await waitFor(() => {
        // First annotation
        const heading1 = screen.getByRole('heading', { level: 1, name: 'First Annotation' });
        expect(heading1).toBeInTheDocument();
        const boldText = screen.getByText('bold', { selector: 'strong' });
        expect(boldText).toBeInTheDocument();

        // Second annotation
        const heading2 = screen.getByRole('heading', { level: 2, name: 'Second Annotation' });
    ...
    it('should handle mixed content types (text and file annotations)', async () => {
      ...
      await waitFor(() => {
        ...
        const boldText = screen.getByText('Text annotation', { selector: 'strong' });
        expect(boldText).toBeInTheDocument();
    ...
    it('should maintain markdown rendering when annotations are reloaded', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('heading', { name: 'Initial Content' })).toBeInTheDocument();
  ...
  describe('Theme Switching', () => {
    it('should update data-color-mode when theme changes from light to dark', async () => {
      ...
      await waitFor(() => {
        const container = screen.getByText('Theme Test').closest('[data-color-mode]');
    ...
    it('should apply theme consistently across multiple annotations', async () => {
      ...
      await waitFor(() => {
        const containers = screen.getAllByText(/First|Second/).map(
          el => el.closest('[data-color-mode]')
        );
  ...
  describe('Performance and Edge Cases', () => {
    it('should handle large number of annotations efficiently', async () => {
      ...
      const mockAnnotations: Annotation[] = Array.from({ length: 20 }, (_, i) => ({
        id: `perf-${i}`,
        gps_point_id: mockPointId,
        type: 'text' as const,
