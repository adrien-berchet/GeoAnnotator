    ...
    it("should sanitize script tags", async () => {
      ...
      await waitFor(() => {
        ...
        const maliciousScripts = Array.from(scriptTags).filter((script) =>
          script.textContent?.includes("XSS Attack"),
        );
...
[Safe link](https://example.com) and [Bad link](javascript:alert('XSS 3'))
      ...
      await waitFor(() => {
        ...
        const annotationDesc = document.querySelector(
        ...
        expect(annotationDesc?.querySelectorAll("[onerror]").length || 0).toBe(
          0,
        );
        expect(annotationDesc?.querySelectorAll("[onclick]").length || 0).toBe(
          0,
        );

        // Safe content should still render
        expect(
          screen.getByRole("heading", { name: "Normal Heading" }),
        ).toBeInTheDocument();
        expect(
  ...
  describe("Security Edge Cases", () => {
    it("should handle extremely long malicious input", async () => {
      const longMaliciousContent =
        "<script>" + 'alert("XSS");'.repeat(1000) + "</script>";
      ...
      await waitFor(() => {
        ...
        ).filter((s) => s.textContent?.includes("XSS"));
    ...
    it("should sanitize nested malicious tags", async () => {
      const mockAnnotations: Annotation[] = [
        {
          ...
          type: "text",
          text_content:
            '<div><script><script>alert("Nested XSS")</script></script></div>',
          file: null,
          order: 1,
          created_at: "2025-10-15T10:00:00Z",
          is_trashed: false,
      ...
      await waitFor(() => {
        const scriptTags = Array.from(
          document.querySelectorAll("script"),
        ).filter((s) => s.textContent?.includes("Nested XSS"));
        expect(scriptTags).toHaveLength(0);
      });
    });
  });
