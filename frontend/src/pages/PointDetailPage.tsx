...
import {
  AnnotationForm,
  type AnnotationType,
} from "../components/annotations/AnnotationForm";
...
export function PointDetailPage() {
  ...
  const handleDeletePoint = async () => {
    ...
    const message = t(
      "pointDetail.confirmDelete",
      'Are you sure you want to delete "{title}"?',
    ).replace(
      "{title}",
      point.title || t("pointDetail.thisPoint", "this point"),
    );
  ...
  if (isLoading) {
    return (
      <LoadingSpinner
        size="large"
        message={t("pointDetail.loadingPoint", "Loading point...")}
      />
    );
  }
    ...
    <div className="point-detail-page">
      ...
      <div className="point-detail-header">
        ...
        <div className="header-actions">
          ...
          >
            {isDeleting
              ? `🗑️ ${t("pointDetail.deleting", "Deleting...")}`
      ...
      <div className="point-info-card">
        <div className="point-info-header">
          <h1>
            {point.title || t("pointDetail.untitledPoint", "Untitled Point")}
          </h1>
          {point.is_public && (
            <span className="public-badge">
              🌐 {t("pointDetail.public", "Public")}
            </span>
          )}
        </div>
        ...
        <div className="point-metadata">
          <div className="metadata-item">
            <span className="metadata-label">
              📍 {t("pointDetail.location", "Location")}
            </span>
          ...
          {point.type && (
            <div className="metadata-item">
              ...
              <span className={`metadata-value ${point.shared_by ? "shared" : ""}`}>
                {point.type.icon && (
                  <img
                    src={point.type.icon}
                    alt=""
                ...
                {point.shared_by && (
                  <span className="shared-indicator" title={`From ${point.shared_by}`}>
                    {" "}🔒
                  </span>
                )}
              </span>
        ...
        {point.tags && point.tags.length > 0 && (
          <div className="point-tags">
            ...
            <div className="tags-list">
              {point.tags.map((tag) => (
                <span key={tag.id} className={`tag ${point.shared_by ? "shared" : ""}`}>
                  {tag.name}
                </span>
              ))}
            </div>
      ...
      <div className="annotations-section">
        <h2>
          {t("pointDetail.annotations", "Annotations")} ({annotations.length})
        </h2>
        ...
        <div className="add-annotation-buttons">
          <button
            ...
            onClick={() =>
              setSelectedAnnotationType(
                selectedAnnotationType === "text" ? null : "text",
              )
            }
          >
          ...
          <button
            ...
            onClick={() =>
              setSelectedAnnotationType(
                selectedAnnotationType === "image" ? null : "image",
              )
            }
          >
          ...
          <button
            ...
            onClick={() =>
              setSelectedAnnotationType(
                selectedAnnotationType === "document" ? null : "document",
              )
            }
          >
          ...
          <button
            ...
            onClick={() =>
              setSelectedAnnotationType(
                selectedAnnotationType === "file" ? null : "file",
              )
            }
          >
