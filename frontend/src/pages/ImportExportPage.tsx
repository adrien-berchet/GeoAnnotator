...
export function ImportExportPage() {
  ...
  const [mergeStrategy, setMergeStrategy] =
    useState<MergeStrategy>("create_new");
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      alert(
        t(
          "importExport.exportError",
          "Failed to export points. Please try again.",
        ),
      );
    } finally {
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await importPoints(
        selectedFile,
        importFormat,
        mergeStrategy,
      );
    ...
    <div className="import-export-page">
      <div className="page-header">
        ...
        <p className="page-description">
          {t(
            ...
            "Export your points and annotations to various formats, or import data from existing files.",
          )}
        </p>
      </div>
      ...
      <div className="import-export-container">
        ...
        <section className="export-section">
          ...
          {(exportFormat === "geojson" || exportFormat === "zip") && (
            <div className="form-group checkbox-group">
              <label>
                ...
                />
                <span>
                  {t("importExport.includeAnnotations", "Include annotations")}
                </span>
              </label>
              <p className="field-hint">
                {exportFormat === "zip"
                  ? t(
                      "importExport.zipHint",
                      "ZIP format includes annotation files in a separate folder",
                    )
                  : t(
                      "importExport.annotationsHint",
                      "Include annotation metadata in the export",
                    )}
              </p>
            </div>
          )}
          ...
          <div className="form-group">
            <label>{t("importExport.selectPoints", "Points to Export")}</label>
            <div className="radio-group">
              <label>
                ...
                />
                <span>
                  {t("importExport.exportAll", "Export all points")} (
                  {points.length})
                </span>
              </label>
              <label>
                ...
                />
                <span>
                  {t("importExport.exportSelected", "Export selected points")} (
          ...
          <button
            ...
            disabled={
              isExporting || (!exportAll && selectedPoints.length === 0)
            }
          ...
          <div className="form-group">
            ...
            <p className="field-hint">
              {mergeStrategy === "create_new" &&
                t(
                  "importExport.createNewHint",
                  "All points will be imported as new, even if similar points exist",
                )}
              {mergeStrategy === "skip" &&
                t(
                  "importExport.skipHint",
                  "Points within 1 meter of existing points will be skipped",
                )}
              {mergeStrategy === "replace" &&
                t(
                  "importExport.replaceHint",
                  "Existing points within 1 meter will be updated with new data",
                )}
            </p>
          </div>

          <div className="form-group">
            <label>{t("importExport.selectFileLabel", "Select File")}</label>
              ...
              <div className="result-stats">
                <div className="stat">
                  <span className="stat-label">
                    {t("importExport.totalPoints", "Total Points")}:
                  </span>
                  <span className="stat-value">
                    {importResult.total_points}
                  </span>
                </div>
                <div className="stat success">
                  <span className="stat-label">
                    {t("importExport.imported", "Imported")}:
                  </span>
                  <span className="stat-value">
                    {importResult.imported_points}
                  </span>
                </div>
                {importResult.skipped_points > 0 && (
                  <div className="stat warning">
                    <span className="stat-label">
                      {t("importExport.skipped", "Skipped")}:
                    </span>
                    <span className="stat-value">
                      {importResult.skipped_points}
                    </span>
                  </div>
                )}
                {importResult.failed_points > 0 && (
                  <div className="stat error">
                    <span className="stat-label">
                      {t("importExport.failed", "Failed")}:
                    </span>
                    <span className="stat-value">
                      {importResult.failed_points}
                    </span>
                  </div>
                )}
              </div>
                ...
                <div className="import-errors">
                  ...
                  <ul>
                    {importResult.errors.slice(0, 10).map((error, idx) => (
                      <li key={idx}>
                        <strong>
                          {t("importExport.line", "Line")} {error.line_number}:
                        </strong>{" "}
                        {error.message}
                      </li>
                    ))}
