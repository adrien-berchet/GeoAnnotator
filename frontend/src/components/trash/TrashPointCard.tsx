...
export function TrashPointCard({
  item,
  onRestore,
  onDelete,
}: TrashPointCardProps) {
  ...
  const handleRestore = async () => {
    if (
      !confirm(t("trash.confirmRestorePoint", "Restore this point from trash?"))
    )
      return;
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        t(
          "trash.confirmDeletePoint",
          "Permanently delete this point? This action is irreversible and will also delete all associated annotations.",
        ),
      )
    ) {
      return;
    ...
    <div className="trash-card trash-point-card">
      ...
      <div className="trash-card-details">
        ...
        <div className="detail-item">
          ...
          <span>
            {item.gps_point.tags.length}{" "}
            {item.gps_point.tags.length === 1
              ? t("common.tag", "tag")
              : t("common.tags", "tags")}
          </span>
        </div>
        ...
        <div className="trash-card-expanded">
          {item.annotations.length > 0 && (
            <div className="expanded-section">
              <h4>
                {t("trash.associatedAnnotations", "Associated annotations")}
              </h4>
          ...
          {item.shares.length > 0 && (
            <div className="expanded-section">
                ...
                {item.shares.map((share) => (
                  <div key={share.id} className="share-item">
                    ...
                    <span className="share-permission">
                      {translatePermission(share.permission)}
                    </span>
                  </div>
                ))}
              </div>
              <p className="shares-note">
                ℹ️{" "}
                {t(
                  "trash.sharesNoAccessNote",
                  "These people no longer have access to the point and its annotations.",
                )}
              </p>
            </div>
          )}
      ...
      {item.is_expired && (
        <div className="expired-notice">
          ⚠️{" "}
          {t(
            "trash.pointExpiredNotice",
            "This point has expired and will be automatically deleted.",
          )}
        </div>
      )}
    </div>
